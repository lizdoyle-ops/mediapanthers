...
const customers = {
  "leyton@finalproduction.club": {
    ...
    brand: "Media Panthers",
    language: "English",
    joined: "2024-03-15",
    lifetime_value: 1240,
    last_login: "2026-06-02 09:14",
    kyc_status: "verified",
...
function fmtBalance(c) {
  ...
  const frozen = c.account_status === "suspended" ? " (frozen)" : "";
  return `€${c.account_balance.toLocaleString()}${frozen}`;
}

// ── Routes ────────────────────────────────────────────────────────────────────

// GET /customers/:email/account
app.get("/customers/:email/account", (req, res) => {
  const email = resolve(req, res);
  if (!email) return;
  ...
  res.json({
    ...
    language: c.language,
    joined: c.joined,
    lifetime_value: `€${c.lifetime_value.toLocaleString()}`,
    last_login: c.last_login,
    kyc_status: c.kyc_status,
    account_balance: fmtBalance(c),
...
// GET /customers/:email/transactions?limit=5
...
// GET /promotions/active?brand=&customer=
...
// POST /customers/:email/account/review
app.post("/customers/:email/account/review", (req, res) => {
  const email = resolve(req, res);
  if (!email) return;
  const review_id = `rev_${Math.random().toString(36).slice(2, 10)}`;
  res.json({ ok: true, review_id, status: "pending", message: "Compliance review triggered" });
});

// PATCH /customers/:email/account
app.patch("/customers/:email/account", (req, res) => {
  ...
  if (account_status) {
    ...
    if (account_status.toLowerCase() === "active") {
      customers[email].suspension_reason = null;
    }
  }
  res.json({ ok: true, account_status: customers[email].account_status });
});

// POST /customers/:email/transactions/retry
app.post("/customers/:email/transactions/retry", (req, res) => {
  const email = resolve(req, res);
  if (!email) return;
  const { method } = req.body;
  const prevFailed = (transactions[email] || [])
  ...
  const newTx = {
    ...
    date: now,
    amount: failedTx ? failedTx.amount : 50,
    currency: "EUR",
    method,
...
// POST /customers/:email/promotions/apply
...
app.listen(PORT, () => console.log(`Customer 360 API → http://localhost:${PORT}`));
